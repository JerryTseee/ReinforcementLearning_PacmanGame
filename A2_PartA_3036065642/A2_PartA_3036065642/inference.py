...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1] # add variables
    edges = [(PAC, OBS0), (PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS1)] # add edges
    possible_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)] # add possible positions
    # assign possible positions (domain) to each variable
    variableDomainsDict[PAC] = possible_positions
    variableDomainsDict[GHOST0] = possible_positions
    variableDomainsDict[GHOST1] = possible_positions

    max_distance = (X_RANGE - 1) + (Y_RANGE - 1)
    variableDomainsDict[OBS0] = [i for i in range(0, max_distance + MAX_NOISE + 1)] # possible positions for observations
    variableDomainsDict[OBS1] = [i for i in range(0, max_distance + MAX_NOISE + 1)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        table = bayesNet.getAllCPTsWithEvidence(evidenceDict) # get all factors with evidence

        for var in eliminationOrder: # iterate in elimination order! (until the only query and evidence variables are left)
            table, joinedFactor = joinFactorsByVariable(table, var)

            if len(joinedFactor.unconditionedVariables()) > 1:
                temp = eliminate(joinedFactor, var) # eliminate the variable
                table.append(temp) 

        
        # join the factors together
        # print("table: ", table)
        final_factor = joinFactors(table)
        # print("final_factor: ", final_factor)
        # normalize them, make them sum to 1
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # write a sample() method that randomly picks one of these keys, but not equally - keys with higher numbers should be picked more often
        total = self.total()
        # logic example: if red: 3, blue: 1, green: 0, then total = 4
        # 0-3 = red (75%), 3-4 = blue (25%), green never picked
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        "*** YOUR CODE HERE ***"
        # handle special case
        if ghostPosition == jailPosition: # then the observation is none
            # observation is None with probability 1, 0 anything else
        ...
        if noisyDistance == None: # check again
            return 0
        else:
            prob = busters.getObservationProbability(noisyDistance, manhattan_distance)
            return prob
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        ghost_pos = self.allPositions # all possible ghost positions

        beliefs = self.beliefs
        # iterate through all, and update the beliefs
        for pos in ghost_pos:
            prob = self.getObservationProb(observation, pac_pos, pos, jail_pos)
            beliefs[pos] = beliefs[pos] * prob # update
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # logic here
        # create a new empty distribution for updated beliefs
        # for each possible current position
        #     get probability of moving to each possible new position
        #     multiply the probability of the current position by the probability of moving to the new position
        #     add to the new distribution for the new position
        # replace the beliefs

        newBeliefs = DiscreteDistribution()
        allPossiblePos = self.allPositions

        for oldPos in allPossiblePos:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            oldPosProb = self.beliefs[oldPos] # get the old probability
            for newPos in newPosDist:
                prob = newPosDist[newPos] # get the probability of new position
                newBeliefs[newPos] += oldPosProb * prob
        
        # replace the old beliefs
        ...
        # for oldPos in legalPos:
        #     newPosDist = self.getPositionDistribution(gameState, oldPos)
        #     for newPos, prob in newPosDist.items():
        #         allPossiblePos[newPos] += self.beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(num_of_particles):
            pos_idx = i % len(legal) # get the index of the position (round cycling)
            self.particles.append(legal[pos_idx])
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        # iterate through the particles, and add to the distribution
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in particles:
            prob = self.getObservationProb(observation, pac_pos, particle, jail_pos)
        ...
        for i in range(self.numParticles):
            new_particles.append(weights.sample()) # call the sample function here
